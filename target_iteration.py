...
# employ one process to generate an adversarial sample
def one_process_run(people_list, model_pool, device): 
    f = open("likelihood_single.json")
    ...
    # Generate a adversarial sample
        noise, _, iter_num = cal_adv(os.path.join('images', origin_name), 
            os.path.join('images', likelihood[origin_name]),
        ...
        origin_img = np.array(Image.open(os.path.join('images', origin_name)).resize((112,112),Image.ANTIALIAS), dtype=float)
        ...
        # Save image
        if os.path.exists('adv_images/') is False:
            os.mkdir('adv_images/')
        ex_name = origin_name.split('_')[0]+'/'
        if os.path.exists('adv_images/'+ex_name) is False:
            os.mkdir('adv_images/'+ex_name)
        jpg_img = 'adv_images/'+ ex_name + origin_name
...
if __name__ == '__main__':
    ...
    device_list = ['cuda']
    '''
    task_num_list = [1000,1000]
    if len(task_num_list) != len(device_list):
        raise 'task_num_list is not same as device_list!'
    if np.array(task_num_list).sum() != 2000:
        print(np.array(task_num_list).sum())
        raise 'imgs num is not 2000!'
    ...
    p_pool.join()
    '''
    one_process_run(faces,get_model_pool(device_list[0]),device_list[0] )
